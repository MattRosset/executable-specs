...
| **Skills** (`skills/*`) | this repo | `~/.claude/skills/*`, `~/.cursor/skills/*` | **Install via `npx skills add MattRosset/executable-specs`** (any supported agent; the CLI asks project vs global) or as a Claude Code plugin (`/plugin install executable-specs`). `sync-skills-to-claude.ps1` is the author-side mirror for editing skills in place — it is not the install path, and it overwrites same-named skill directories. |
| **Global standing rules** (`starters/global-standing-rules.md`) | my private `~/.claude/CLAUDE.md` / playbook | this repo (genericized public snapshot); each machine's `~/.claude/CLAUDE.md` | Genericized on export (lab specifics + private playbook path dropped, per the checklist below). A plugin **cannot** write the global `~/.claude/CLAUDE.md` (a CLAUDE.md at plugin root does not load) and there is no native cross-machine memory sync, so this file is consumed by a manual `@import` line the user adds once per machine — versioned here, `git pull` propagates. The deployed original stays firm/verbatim; the public copy is the genericized one. |
