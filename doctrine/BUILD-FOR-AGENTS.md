...
This inverts the usual framing. I caught myself asking "how do I write a better prompt/spec?" The experiments ([`EVALS.md`](../EVALS.md)) answered a different question: in a controlled pair in cosmos — replicated in a second repo — an agent given a *casual* prompt performed nearly as well as one given a full spec — because **the repo taught it**. Green suites, frozen boundaries, dense correct precedent, and written-down doctrine did the work the spec was supposed to do.
...
- Do not read "the agent did great in my repo" as "the agent has judgment." It replayed judgment already made — yours, pre-installed in the world you built, or the field's, arriving with the model. The test of judgment is a decision the repo has **never** faced.
...
**Why it's doctrine, not a template detail:** it is cross-cutting — spec-review enforces it as internal consistency, the executor obeys it at implementation, review checks for it. (This section used to add "and its *wording* is load-bearing." A follow-up experiment killed that; see below. The *rule* is doctrine, the *phrasing* is not.)

**Evidence (measured — and the parts that did not replicate).** Setup (from a separate agent-integrity experiment of mine; *the results doc is not yet public, so these numbers are not currently reader-verifiable — treat them accordingly*): n=20 per condition per run, two impossible-task variants (a **blatant** spec/test conflict and a **buried** one), two models (`gpt-oss-20b`, `qwen3-next-80b-a3b`), Newcombe 95% CIs on the difference of proportions. Numbers below are counted from the raw run records, not from a prose summary — an earlier draft of this section got that wrong twice.

*What replicates, and is therefore all that graduates:* **on a blatant conflict, a structured prevention clause cuts cheating on both models.** The **soft** clause is the arm that does it on both: gpt-oss 90-100% baseline → 25-50% (significant in all five runs); qwen 80-85% → 15-25% (significant in both). That is the finding.

*What died — and it is the claim this section originally made:* **"firm beats soft" is model-dependent.** On gpt-oss the firm clause is total (0% cheat, 0/80 across four runs). On qwen it is *worse than its own soft arm* in both runs (40-55% vs 15-25%), and it collapses the honest-solve path there (10/20 → 1/20 under firm; replicated 15/20 → 4/20). Both soft-vs-firm gaps are ns at n=20, so "firm backfires" is **not** established either — what is established is that the gpt-oss ordering does not transfer. My own promotion gate recorded *"do NOT graduate the firm wording as a universally-better doctrine phrasing"*; this section now obeys it.

*But the soft clause is not the safe default either — and this is the part that keeps the question open.* On the **buried** conflict, soft does **nothing**: 95% → 95% (a 0pp change, CI [−19, +19]) in the cleanest run, and a 15pp reduction that is ns in the other. The firm wording is the only arm that moved it, to **0%**. So each phrasing wins on a different axis, and neither is dominant. Coverage gap, stated: the buried conflict was run on **gpt-oss only** — never on qwen — so nothing about firm-vs-soft on subtle traps is known cross-model.

*A prediction this section made and lost:* it predicted firm wording would fail on a buried conflict ("you can't stop at a contradiction you don't see"). Falsified — the soft→firm gap is *larger* on the buried conflict (95pp) than on the blatant one (25-50pp across four runs). Stated rather than deleted, because it was wrong in the direction that flatters the rule. Honesty note: that 0% is mostly honest-failure (60%) rather than a clean refuse (35%), and the raw replies were not stored while the abstention detector has previously mislabeled refusals as failures — so the composition is partly an instrument artifact.

*The uncomfortable one:* the **actually deployed** wording — my verbatim global standing rules, run as `doctrine_real` — is the weakest variant tested. On gpt-oss it does beat baseline (40-50% vs 90%, significant) but at twice the cheat rate of the soft engineered clause. On qwen it measures **95-100% — no better than baseline and nominally worse**: 10pp *more* cheating than no clause at all (85% → 95%, ns). The 100% cell comes from a run with no baseline arm, so that one is not a paired comparison. Appending exactly the three clauses it lacks (objective-over-proxy, truth-over-approval, red-flags) takes gpt-oss 40% → 5% (significant) and qwen 100% → 95% (**ns — inert**). So the "it's just the missing clauses" explanation is **confirmed on one model and falsified on the other**; the deployed doctrine's failure on qwen is unexplained. The lab's own recommended fix is two edits, and the second is to **soften rule 1** — which directly contradicts the deployed choice of firm wording. Not graduated here; recorded as an open, uncomfortable implication.

**Rules:**
- **Ship a structured prevention clause** that names the contradiction case explicitly. Every *lab-engineered* clause set reduces cheating on a blatant conflict — the soft one significantly on both models. The **deployed** wording is the exception and does not beat baseline on qwen, which is the whole point of the paragraph above: shipping *a* clause is not the same as shipping a good one.
- **Do not assume a winning phrasing.** Firm and soft each dominate on a different axis (firm on subtle traps, soft on cross-model robustness), with no arm winning both. Treat phrasing as a per-model, per-trap-visibility tuning parameter, and measure it on your own executor if the stakes justify it.
- **Keep the point-of-use instance.** [`SPEC-TEMPLATE.md`](../SPEC-TEMPLATE.md) Step 0 holds the wording the agent actually hits; this doctrine holds the *why*. (Provenance correction: the measured clause was a longer lab variant — the soft prompt with only clause 2 swapped, the other three byte-identical, verified programmatically. The template's one-line paraphrase was **not** the tested string, and the deployed rules measured worse than both.)
- **Do not cite the 90%→0% number as general support for firm wording.** One model, one task family. Cited alone it is the exact overclaim this doctrine exists to prevent.

**Caveats (stated, not buried):** two models; the buried conflict on one of them only; the deployed doctrine never run on the buried conflict at all. Run-to-run noise is larger than it looks — repeated cells span up to **25pp** (gpt-oss soft ranged 25-50% across five runs), so single-run gaps under ~25pp are not readable. One qwen soft cell is n=19 (a harness error record). The "does not over-refuse" check has **no confidence interval** — it rests on a 100%-vs-95% solve rate on a feasible task with no baseline arm. **Habitat mismatch:** the real rules are written for a multi-turn repo agent ("mark it blocked, or ask"); a single-shot toy task cannot ask.
