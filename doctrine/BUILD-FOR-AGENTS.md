...
*But the soft clause is not the safe default either — and this is the part that keeps the question open.* On the **buried** conflict, soft does **nothing**: 95% → 95% (+0pp, CI [−19, +19]) in the cleanest run, +15pp ns in the other. The firm wording is the only arm that moved it, to **0%**. So each phrasing wins on a different axis, and neither is dominant. Coverage gap, stated: the buried conflict was run on **gpt-oss only** — never on qwen — so nothing about firm-vs-soft on subtle traps is known cross-model.

*A prediction this section made and lost:* it predicted firm wording would fail on a buried conflict ("you can't stop at a contradiction you don't see"). Falsified — the soft→firm gap is *larger* on the buried conflict (95pp) than on the blatant one (40pp). Stated rather than deleted, because it was wrong in the direction that flatters the rule. Honesty note: that 0% is mostly honest-failure (60%) rather than a clean refuse (35%), and the raw replies were not stored while the abstention detector has previously mislabeled refusals as failures — so the composition is partly an instrument artifact.

*The uncomfortable one:* the **actually deployed** wording — my verbatim global standing rules, run as `doctrine_real` — is the weakest variant tested. On gpt-oss it does beat baseline (40-50% vs 90%, significant) but at twice the cheat rate of the soft engineered clause. On qwen it measures **95-100%, i.e. no better than baseline and nominally worse** (−10pp, ns). Appending exactly the three clauses it lacks (objective-over-proxy, truth-over-approval, red-flags) takes gpt-oss 40% → 5% (significant) and qwen 100% → 95% (**ns — inert**). So the "it's just the missing clauses" explanation is **confirmed on one model and falsified on the other**; the deployed doctrine's failure on qwen is unexplained. The lab's own recommended fix is two edits, and the second is to **soften rule 1** — which directly contradicts the deployed choice of firm wording. Not graduated here; recorded as an open, uncomfortable implication.

**Rules:**
- **Ship a structured prevention clause** that names the contradiction case explicitly. Every clause set tested beats no clause on a blatant conflict, on both models. That much is solid.
