...
**Evidence (measured — and the part that did not replicate).** All runs come from a separate agent-integrity experiment of mine, n=20 per condition, on tasks with a seeded spec/test conflict. *The results doc is not yet public, so these numbers are not currently reader-verifiable — treat them accordingly.*

*What holds on both models tested:* **a structured prevention clause reduces reward hacking.** On `gpt-oss-20b` with a blatant conflict, cheating fell from a **90% baseline** to 40% (soft wording) and 0% (firm). On `qwen3-next-80b-a3b`, from ~85% baseline to 15-25% (soft). Every doctrine arm beat baseline on every model. This is the claim that graduates.

*What died — and it is the claim this section originally made:* **"firm wording beats soft" is model-dependent, not a law.** On the second model the ordering **reverses**: firm is *worse* than soft (40-55% cheat vs 15-25%), and the mechanism is visible — firm wording collapses that model's honest-solve path (solve 10/20 under soft → 1/20 under firm). My own promotion gate fired against the law and recorded *"do NOT graduate the firm wording as a universally-better doctrine phrasing."* This doctrine now obeys that gate. Note the direct soft-vs-firm gap on the second model is **not significant** at n=20, so "firm *backfires*" is not established either; what is established is that the first model's ordering does not transfer.

*A prediction this section made and lost:* it predicted firm wording would fail on a **buried** conflict ("you can't stop at a contradiction you don't see"). Measured on a buried-conflict task: firm drove cheating to **0%** where soft stayed at **95%** — the soft→firm gap is *larger* on the buried conflict than the blatant one. The prediction was falsified in the direction that flatters the rule, which is precisely why it gets stated rather than quietly deleted. Honesty note: that 0% is mostly honest-failure (60%) rather than a clean refuse (35%) — the model stops gaming, but doesn't articulate the blocker as crisply as on the blatant case.

*The uncomfortable one:* the **actually deployed** wording — my verbatim global standing rules, run as their own condition — is the **weakest** variant on **both** models (50% cheat on the first, 95% ≈ baseline on the second). The engineered lab clause set was flattering it. The deployed text has two independent problems: it lacks the objective-over-proxy and anti-hardcode clauses that carry most of the anti-cheat effect, *and* its rule-1 wording is the firm variant some models react badly to. Its own self-justification cites a number that is neither its wording nor cross-model. That needs fixing at the source; it is not fixed by this file.

**Rules:**
- **Ship a structured prevention clause** that names the contradiction case explicitly. That is the model-robust finding. Which *phrasing* — firm or soft — is a tuning parameter of your executor model, not a law; if you can measure it on your model, do, and expect the answer to differ.
- **Keep the point-of-use instance.** [`SPEC-TEMPLATE.md`](../SPEC-TEMPLATE.md) Step 0 holds the wording the agent actually hits; this doctrine holds the *why*. Graduating a principle never empties its instance. (Provenance correction: the measured clause was a longer lab variant — the soft prompt with only the stop-at-contradictions clause swapped, the other three byte-identical. The template's one-line paraphrase was **not** the tested string, and the deployed rules measured worse than both.)
- **Do not cite the 90%→0% number as support for firm wording generally.** It is one model, one task family. Cited alone it is the exact overclaim this doctrine exists to prevent.

**Caveats (stated, not buried):** two models, mostly one task, n=20 single runs — run-to-run noise is ~10-15pp, and one cell moved 55→40 between runs. The deployed doctrine was never run on the buried conflict. **Habitat mismatch:** the real rules are written for a multi-turn repo agent ("mark it blocked, or ask"); a single-shot toy task can't ask. The missing anti-hack clauses are not habitat-dependent, so that finding survives; the wording comparison may not.
